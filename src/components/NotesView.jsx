...
function NoteRow({ note, onEdit, onDelete, onOpen, onSelect, active = false }) {
  ...
  return <article className={`document-row editable-row ${active ? 'active' : ''}`} onClick={onSelect}><span className={`file-icon ${note.type === 'DOC' ? 'doc' : ''}`}>{note.type}</span><div><strong>{note.title}</strong><small>{note.subject} - {formatDate(note.date)} - {note.size} - {storageLabel}</small>{note.fileName ? <em>{note.fileName}</em> : null}</div><div className="row-actions">{note.filePath ? <button type="button" onClick={(event) => { event.stopPropagation(); onOpen?.(); }} aria-label="Abrir archivo"><ExternalLink size={17} /></button> : null}<button type="button" onClick={(event) => { event.stopPropagation(); onEdit(); }} aria-label="Editar"><Pencil size={17} /></button><button type="button" onClick={(event) => { event.stopPropagation(); onDelete(); }} aria-label="Borrar"><Trash2 size={17} /></button></div></article>;
}
