...
const assistantFileWarning = 'El asistente IA puede analizar PDF y DOCX. Si tu archivo es .doc antiguo, guardalo como DOCX o PDF y volvelo a subir.';
const canUseAssistantFile = (note) => {
  const name = note?.fileName?.toLowerCase() || '';
  return Boolean(note?.filePath && (note?.type === 'PDF' || name.endsWith('.pdf') || name.endsWith('.docx')));
};
